...
from dfa.agent.vdp import dfa_vdp_mgr
...
class DfaVdpMgrTest(base.BaseTestCase):
    ...
    def _test_dfa_mgr_init(self):
        ...
        with contextlib.nested(
            mock.patch('dfa.common.utils.EventProcessingThread'),
            mock.patch('dfa.common.utils.PeriodicTask'),
        ) as (event_fn, period_fn):
    ...
    def _test_process_up_uplink_event(self):
        ...
        with contextlib.nested(
            mock.patch('dfa.agent.vdp.ovs_vdp.OVSNeutronVdp'),
            mock.patch.object(self.dfa_vdp_mgr, 'save_uplink'),
        ) as (ovs_vdp_fn, save_uplink_fn):
